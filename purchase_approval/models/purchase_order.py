...
class PurchaseOrder(models.Model):
    ...
    current_approver = fields.Many2one(
        comodel_name="purchase.order.approver", string="Current Approver",
        ...
        for order in self:
            if not order.team_id:
                ...
                if order.current_approver.user_id == self.env.user or self.env.is_superuser():
                    ...
                    #     ('res_id', '=', self.id),
                    #     ('user_id', '=', order.current_approver.user_id.id),
                    # ])
                    ...
                    if order.next_approver:
                        ...
                        order._create_notification_activity(
                            _("Purchase Order %s is waiting for your approval.") % order.name
                        )
                    else:
        ...
        for order in self:
            ...
            if not order.team_id:
                ...
                if order.next_approver:                    
                    ...
                    order.send_to_approve()
                    # Create activity for the next approver to approve the purchase order 
    ...
    # call super in button_draft method to reset approval route when resetting to draft
    ...
    def _create_notification_activity(self, message):
        activity_type = self.env.ref("mail.mail_activity_data_todo")
        model_id = self.env["ir.model"]._get("purchase.order").id
        date_deadline = self.date_order or fields.Date.today()
        next_approval = self.current_approver.user_id

        if next_approval:
            next_approval_user = next_approval.user_id
            # if next_approval_user.id != user_id:
            context = dict(
                self.env.context,
                mail_activity_quick_update=True,
                mail_notify_force_send=False,
            )
            activity = self.env["mail.activity"].sudo().with_context(context).create(
                {
                    "activity_type_id": activity_type.id,
                    "res_model_id": model_id,
                    "res_id": self.id,
                    "user_id": next_approval.id,
                    "summary": _("Next approval for Purchase Order %s") % self.name,
                    "note": _("Please approve the purchase order."),
                    "automated": True,
...
class PurchaseReport(models.Model):
    ...
    def _select(self):
        return SQL("%s, po.team_id as type_id", super()._select())

    def _group_by(self):
        return SQL("%s, type_id", super()._group_by())